...
class EvenLegendreTerms(SimpleCLLibrary):
    def __init__(self):
        ...
        This routine calculates Pn(x) for all n in [0, 2, 4, ..., n] using the following recursion:
        ...
        """
        super().__init__('''
            void EvenLegendreTerms(double x, int n, double* legendre_terms){
                if(fabs(x) == 1.0){
                    // the legendre terms collapse to 1 for even terms if fabs(x) == 1.0
                    for(int i = 0; i < n; i++){
