...
class EvaluateModel(AbstractCLRoutine):
    ...
    def calculate(self, model, parameters):
        ...
        errors = np.asmatrix(np.zeros((nmr_problems, nmr_inst_per_problem)).astype(np.float64, order='C', copy=False))
        ...
        def run_transformer_cb(cl_environment, start, end, buffered_dicts):
            ...
            kernel_source = self._get_kernel_source(
                model.get_model_eval_function('evaluateModel'),
                parameters.shape[1],
                nmr_inst_per_problem,
                problems_var_data_dict,
                problems_prtcl_data_dict,
    ...
    def _run_calculator(self, parameters, var_data_dict, errors_host, prtcl_data_buffers, fixed_data_buffers,
        ...
        evals_buf = cl.Buffer(cl_environment.context, write_only_flags, hostbuf=errors_host[start:end, :])
        ...
        event = cl.enqueue_copy(queue, errors_host[start:end, :], evals_buf, is_blocking=False)

        return queue, event

    def _get_kernel_source(self, cl_func, nmr_params, nmr_inst_per_problem, var_data_dict,
