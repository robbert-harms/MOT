...
class CalculateDependentParameters(AbstractCLRoutine):
    ...
    def calculate(self, estimated_parameters_list, parameters_listing, dependent_parameter_names):
        ...
        workers = self._create_workers(_CDPWorker, estimated_parameters_list,
...
class _CDPWorker(Worker2):
    ...
    def __init__(self, cl_environment, estimated_parameters_list, parameters_listing,
        ...
        self._estimated_parameters = set_correct_cl_data_type(
            np.dstack(estimated_parameters_list).ravel().reshape((-1, 1)))
